(() => {
  const dataNode = document.getElementById('nfl-map-data');
  ...
  try {
    ...
    console.error('Could not read NFL stadium map data.', error);
    return;
  }
  ...
  function sectionLevel(section) {
    ...
    if (/\b(field|floor|pit)\b/.test(normalized) && number === null) {
      return { key: 'field', label: 'Field level', order: 50 };
    }
    ...
    if (/\b(upper|terrace|gridiron)\b/.test(normalized)) {
      return { key: 'upper', label: 'Upper level', order: 750 };
    }
  ...
  function formatPrice(section) {
    return section.price === null
      ? 'No current price'
      : `$${Math.round(section.price).toLocaleString('en-US')}`;
  }
  ...
  function drawField() {
    ...
    const fieldGroup = createSvgElement('g', { 'aria-hidden': 'true' });
    fieldGroup.appendChild(createSvgElement('rect', {
      class: 'nfl-map-field-shell',
      x: 340,
      y: 250,
      width: 320,
      height: 200,
    ...
    for (let index = 1; index < 10; index += 1) {
      const x = 382 + (236 / 10) * index;
      fieldGroup.appendChild(createSvgElement('line', {
        class: 'nfl-map-field-line',
        x1: x,
        y1: 250,
        x2: x,
        y2: 450,
    ...
    for (let index = 0; index < 20; index += 1) {
      const x = 388 + (224 / 19) * index;
      fieldGroup.appendChild(createSvgElement('line', {
        class: 'nfl-map-field-hash',
        x1: x,
        y1: 315,
        x2: x,
        y2: 326,
  ...
  function selectSection(section, level) {
    ...
    if (historyLink) {
      ...
      historyLink.href = `${mapData.graph_url || '/nfl/graph'}?${query.toString()}`;
  ...
  if (stage) {
    stage.addEventListener('wheel', (event) => {
      event.preventDefault();
      setZoom(zoom * (event.deltaY < 0 ? 1.16 : .86), event);
    }, { passive: false });
    ...
    stage.addEventListener('pointermove', (event) => {
      ...
      const bounds = stage.getBoundingClientRect();
      centerX = dragState.centerX - (event.clientX - dragState.x) * currentView.width / bounds.width;
      centerY = dragState.centerY - (event.clientY - dragState.y) * currentView.height / bounds.height;
      applyViewBox();
    });
