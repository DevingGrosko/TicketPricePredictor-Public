...
document.querySelectorAll('.interactive-chart').forEach((chart) => {
  ...
  const yAxisLabel = chart.dataset.yAxisLabel || (mode === 'money' ? 'Average listed price' : 'Relative price');
  ...
  addSvgElement(svg, 'text', { x: 18, y: margin.top + plotHeight / 2, 'text-anchor': 'middle', transform: `rotate(-90 18 ${margin.top + plotHeight / 2})`, class: 'interactive-chart__axis-label' }, yAxisLabel);
