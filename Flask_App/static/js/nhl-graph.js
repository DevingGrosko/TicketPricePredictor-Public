...
function formatHoursBeforePuckDrop(hours) {
  if (hours < 1) return `${Math.round(hours * 60)} min before puck drop`;
  const precision = hours < 10 ? 1 : 0;
  return `${hours.toFixed(precision)} hours before puck drop`;
}
...
document.querySelectorAll('.interactive-chart').forEach((chart) => {
  ...
  const currency = String(chart.dataset.currency || 'USD').toUpperCase();
  const yAxisLabel = chart.dataset.yAxisLabel
    || (mode === 'money' ? `Lowest observed price (${currency})` : 'Relative price');
  ...
  const width = 900;
  const height = 520;
  const margin = { top: 28, right: 28, bottom: 70, left: 86 };
  ...
  const scaleX = (value) => margin.left
    + ((xMax - value) / Math.max(xMax - xMin, 1)) * plotWidth;
  const scaleY = (value) => margin.top
    + ((yMax - value) / Math.max(yMax - yMin, 1)) * plotHeight;
  const formatValue = (value) => {
    if (mode !== 'money') return `${Math.round(value)}%`;
  ...
  for (let index = 0; index <= 4; index += 1) {
    ...
    addSvgElement(gridGroup, 'line', {
      x1: margin.left,
      y1: yPosition,
      x2: width - margin.right,
      y2: yPosition,
      class: 'interactive-chart__grid',
    });
    addSvgElement(gridGroup, 'text', {
      x: margin.left - 13,
      y: yPosition + 4,
      'text-anchor': 'end',
      class: 'interactive-chart__tick',
    }, formatValue(value));
  }
  ...
  addSvgElement(svg, 'text', {
    ...
    y: height - 13,
    'text-anchor': 'middle',
    class: 'interactive-chart__axis-label',
  }, 'Hours until puck drop');
  addSvgElement(svg, 'text', {
    x: 18,
  ...
  const linePath = plottedPoints
    .map((point, index) => `${index ? 'L' : 'M'} ${point.plotX} ${point.plotY}`)
    .join(' ');
  const areaPath = `${linePath} L ${plottedPoints.at(-1).plotX} ${margin.top + plotHeight} `
    + `L ${plottedPoints[0].plotX} ${margin.top + plotHeight} Z`;
  ...
  const guide = addSvgElement(svg, 'line', {
    y1: margin.top,
    y2: margin.top + plotHeight,
    class: 'interactive-chart__guide',
    visibility: 'hidden',
  });
  ...
  const showTooltip = (point, circle, clientX, clientY) => {
    ...
    tooltip.innerHTML = `<strong>${formatValue(point.y)}</strong>`
      + `<span>${formatHoursBeforePuckDrop(point.x)}</span>`;
  ...
  const circles = plottedPoints.map((point) => {
    const circle = addSvgElement(svg, 'circle', {
      ...
      r: 4.5,
      tabindex: 0,
      class: 'interactive-chart__point',
      'aria-label': `${formatValue(point.y)}, ${formatHoursBeforePuckDrop(point.x)}`,
    });
  ...
  svg.addEventListener('pointermove', (event) => {
    ...
    const closestIndex = plottedPoints.reduce((bestIndex, point, index) => (
      Math.abs(point.plotX - pointerX) < Math.abs(plottedPoints[bestIndex].plotX - pointerX)
        ? index
        : bestIndex
    ), 0);
    showTooltip(
      plottedPoints[closestIndex],
      circles[closestIndex],
      event.clientX,
      event.clientY,
    );
  });
